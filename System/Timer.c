...
void Timer_Init(void)
{
	...
	TIM_TimeBaseInit(TIM4, &TIM_TimeBaseInitStructure);//prescaler是预分频值，晶振分频值后计数值加一，分频越多计数越慢
	
	/* Enable timer update IT */
	TIM_ClearFlag(TIM4, TIM_FLAG_Update);//这一句是在初始化后清零标志位，从零开始计数
	...
	NVIC_InitStructure.NVIC_IRQChannel = TIM4_IRQn;//再初始化各种参数
...
//void TIM4_IRQHandler(void)//这个中断函数在启动文件里找,而且中断函数可以放在使用它的地方
//{
